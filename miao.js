(function(G, M) {
  ...
  Object.defineProperties(miao, {
    'VERSION': { get: function() {
        return '0.1.8.0' }, enumerable: true },
    'uid': { get: function() {
        var n = 0;
        return function() {
          return ++n } }() }
  ...
  miao.fn = fn = {
    ...
    isS: function(a) {
      return Object[pes].toString.call(a) == '[object String]' },
    isF: function(a) {
      return Object[pes].toString.call(a) == '[object Function]' },
    isO: function(a) {
      return Object[pes].toString.call(a) == '[object Object]' },
    isN: function(a) {
      return Object[pes].toString.call(a) == '[object Number]' },
    isA: function(a) {
      return Object[pes].toString.call(a) == '[object Array]' },
    getLength: function(a) {
      var i, cont = 0;
      for (i in a) cont++;
      return cont },
    coverOwn: function(a, b) {
      for (var i in b) a[i] = b[i];
      return a },
    setOwn: function(a, b, c) {
      for (var i in b)(a[i] !== uf && !fn.isF(a[i])) && (a[i] = b[i]);
      return a },
    ...
    addOwn: function(a, b) {
      for (var i in b) a[i] === uf && (a[i] = b[i]);
      return a },
    setCss: function(a, b) {
      for (var i in b) a.style[i] = b[i];
      return a },
    find: function(a, b) {
    ...
    getAtyView: function() {
      return miao.app.atyView },
    setAtyView: function(a) { miao.app.atyView = a },
    inof: function(a, b) {
      return a instanceof fn.find('class', b) },
    getPix: function(a) {
    ...
    collision: function(a, b, c) {
      if (a == 'pr') {
        var _x = c.x - c.anchorX * c.w * c.scaleX,
          _y = c.y - c.anchorY * c.h * c.scaleY;
        return !(b.x < _x || b.x > _x + c.w * c.scaleX || b.y < _y || b.y > _y + c.h * c.scaleY)
      } else if (a == 'rr') {
        return Math.powbs(b.x + b.w / 2 - c.x - c.w / 2) < Math.powbs((b.w + c.w) / 2) &&
          Math.powbs(b.y + b.h / 2 - c.y - c.h / 2) < Math.powbs((b.h + c.h) / 2)
      }
    }
  };
  ...
  ! function(Ease) {
    ...
    Ease.getaIn = function(a) {
      return function(t) {
        return Math.pow(t, a) } }

    Ease.getaOut = function(a) {
      return function(t) {
        return 1 - Math.pow(1 - t, a) } }

    Ease.getaInOut = function(a) {
      return function(t) {
        if ((t *= 2) < 1) return 0.5 * Math.pow(t, a);
        return 1 - 0.5 * Math.powbs(Math.pow(2 - t, a))
      }
    }
    ...
    Ease.bounceOut = function(t) {
      if (t < 1 / 2.75) {
        return (7.5625 * t * t) } else if (t < 2 / 2.75) {
        return (7.5625 * (t -= 1.5 / 2.75) * t + 0.75) } else if (t < 2.5 / 2.75) {
        return (7.5625 * (t -= 2.25 / 2.75) * t + 0.9375) } else {
  ...
  var tween = miao.tween = fn.cO({
    ...
    //stop:nl,//是否执行回掉，回到动画前 还是停止 还是直接完成动画，是否继续后面的动画
    update: function() {
        ...
        for (; i < ql; i++) {
          ...
          if (me.stop) {
            ...
            me.stop.isbak !== uf && (me.stop.isbak ? fn.setOwn(me.hok, me.stos) :
              ! function() {
                for (l in me.stos) me.hok[l] = me.tos[l] }());
            ...
            if (me.ia) {
              ...
              if (!me.count)
                for (j in me.tos) me.hok[j] = me.tos[j];
            ...
            /*
  				me.idx&&(++me.idx[0]>=me.idxl[0]&&(me.idx[0]=0));
  				if(me.loop)
  				{
  					if(me.tos)for(j in me.tos)me.hok[j]=me.stos[j];
  					!(me.cak&&me.cak.call(me.hok))&&(me.st = 0);
  				}else{
  					if(me.tos)for(j in me.tos)me.hok[j]=me.tos[j];
  					me.cak&&me.cak.call(me.hok);
  					if(!me.idxl){
  						me.hok.aqe[me.id]=nl;
  						delete me.hok.aqe[me.id]}
					q.splice(i,1);
  				}
  				*/
    ...
    hand: function(a, b) {
      ...
      //c.ty = fn.isA(a.aqe);
      // c.count = a.count ? a.count : a.loop ? 0 : 1 ;
      c.delay ? (c.end = c.delay + (c.time || 0)) : (c.end = (c.time || 0)) && (c.delay = 0);
      return c
    }
  });
  fn.define(tween, 'isrun', { get: function() {
      return !this.queue.length && !this.queuePend.length } });
  ...
  fn.cC(fn.defines({
    ...
    stop: function(a, b) {
      ...
      //if(this.aqe[idx])this.aqe[idx].ahok.stop = a||{};
    },
    ...
    bindTo: function(v) {
      ...
      // if(fn.inof(this,'input'))console.log('v');
    ...
    'hide': {
      get: function() {
        if (this.world && this.world.enabled === false && this.world.visible === false)
          ...
          return false;
        else return this._visible && this._enabled;
      },
      set: function(value) {
  ...
  miao.cas = miao.fn.cO({
    ...
    repay: function(a) {
      ...
      // a.ctx.clearRect(0,0,a.cas.width,a.cas.height);
  ...
  var pulse = fn.cO({
    ...
    start: function() {
      ...
      ! function lp() {
        ...
        miao.app.mlayer && miao.app.mlayer.visible && miao.app.mlayer.draw(miao.app.ctx);
  ...
  fn.cC({
    ...
    sani: function(a) {
      ...
      if (a.spirits) {
        ...
        //this.set({image:a.spirits[0],auto:{w:this.w}});
      }
  ...
  fn.cC(fn.defines({
    ...
    draw: function(ct) {
      ...
      if (!this.opacity == 1 || !this.scaleX == 1 || !this.scaleX == 1 || this.rotation) {
        ...
        // console.log(this.eangle,this.sangle);
        ct.closePath();
    ...
    'eangle': {
      ...
      set: function(value) {
        ...
        // this._eangle/(Math.PI/180)+90 = value-90;
      },
      enumerable: true,
      configurable: true
    }
  ...
  ! function() {
    ...
    // 	pub.trigge(textwho,'chang',textin.value);
    // },false);
  ...
  fn.cC(fn.defines({
  	...
  	constructor:function(a){
  		 ...
  		 var d = this.dom;
  		this.type = d.type = a.type || 'text';
  		fn.setCss(d,{
  			'position': 'fixed',
  			'z-index':101,
  		    opacity:0,
  			fontSize:a.size||(a.h||miao.app.w*.1)*.8+'px',
  			fontFamily:a.font||'Microsoft YaHei'
  		});
  	...
  	'w': {
          ...
          set:function(value){
          	this.dom.style.width = this._w = value;
          },
          enumerable: true,
  	    configurable: true
      }
      ...
      'h': {
          ...
          set:function(value){
          	this.dom.style.height = this._h = value;
          },
          enumerable: true,
          configurable: true
      },
     '_x': {
      ...
      'x': {
  	        ...
  	        set:function(value){
  	        	this.dom.style.left = this._x = value;
  	        },
  	        enumerable: true,
      },
      '_y':{
      ...
      'y':{
  	        ...
  	        set:function(value){
  	        	this.dom.style.top = this._y = value;
  	        },
  	        enumerable: true,
  		    configurable: true
      }
  ...
  */
  fn.cC({
    ...
    addTo: function(vv, b) {
      // var v = fn.inof(vv,'layer')?vv:fn.inof(vv,'view')?vv.mlay:nl;
    ...
    items: function(a) {
      var c = this,
        cak = [],
        pichu = ['group', 'controller'];

      a.forEach(function(m) {
        if (m.ref) {
          //console.log(m.ref,miao(m.ref));
          miao(m.ref).addTo(c);
  ...
  fn.cC({
    ...
    someFn: function() {
      // this.cas = miao.app.cas;
      // this.ctx = miao.app.ctx;
    ...
    var ts = 'changedTouches',
      ev = fn.cO('plug', {
        ...
        axEv: function(n, e) {
          ...
          var av = miao.app.atyView._items,
            i, j;
        ...
        // {
        // 	e.preventDefault();
        // 	miao.dx=(e.pageX?e.pageX-miao.px:(e[ts]&&e[ts][0].pageX||0)-miao.px)-ev.cas.offsetLeft;
        // 	miao.dy=(e.pageY?e.pageY-miao.py:(e[ts]&&e[ts][0].pageY||0)-miao.py)-ev.cas.offsetTop;
        // 	miao.px=(e.pageX?e.pageX:(e[ts]&&e[ts][0].pageX||0))-ev.cas.offsetLeft;
        // 	miao.py=(e.pageY?e.pageY:(e[ts]&&e[ts][0].pageY||0))-ev.cas.offsetTop;
        // 	if(e.type=='mousedown'||e.type=='touchstart')ev.axEv('down',e);else
        // 	if(e.type=='mousemove'||e.type=='touchmove' )ev.axEv('move',e);else
        // 	if(e.type=='mouseup'  ||e.type=='touchend'  )ev.axEv('up'  ,e);
        // },
        ...
        addListener: function(c) {
          ...
          if (!miao.device.isPhone) {
            ...
            c.addEventListener('mouseup', ev.axUp, false)
          } else {
    ...
    var R = fn.cO({
      ...
      loaders: {
        ...
        image: function(_srcs, __ck) {
          ...
          _s.forEach(function(m) {
            ...
            var a = new Image(),
              res = fn.cO('R', { x: 0, y: 0, w: a.width, h: a.height, obj: a });
      ...
      load: function(ra) {
        var oa = {},
          ...
          qu;
        for (i in ra.R)
          if (ra.R[i].length > 0) ars.push(i) && (rLength += ra.R[i].length);
          //console.log('l');
          ! function exe(_i) {
          var m = ts.sx[_i],
            jsle = 0;
          ...
          } else if (idx < ts.sx.length) { exe(idx++) } else if (ars.length > 0) {
    ...
    constructor: function(a) {
      ...
      if (vi = document.querySelector("meta[name=viewport]")) {
        vi.setAttribute('content', 'width=device-width,height=device-height,initial-scale=' +
          (Math.round(innerWidth / (innerWidth * devicePixelRatio) * 100 / this.resolution) / 100) + ',user-scalable=no');
      } else {
  ...
  miao.jsonP = {
    ...
    load: function(url, cs, callback_) {
      var cg = '';
      if (cs)
        for (var i in cs) cg += '&' + i + '=' + cs[i];
      var callbackName = "mjsonp_cak" + miao.uid;
      ...
      sc.src = url + '?callback=' + callbackName + cg;
  ...
  miao.ajax = {
    modo: {
      // 'adds':'http://zglyfqs.shougan.com.cn/action/zglfkcc/zgl.ashx',
      // 'getxx':'http://zglyfqs.shougan.com.cn/action/test.ashx?aa=dd'
    },

    xhr: window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP'),
    load: function(_name, _cs, ajCallbacks) {
      // var pz ='？';
      // if(_cs){
      // 	for(var i in _cs){
      // 		pz += '&' + i + '=' +_cs[i]};
      ...
      this.onreadystatechange(ajCallbacks, this.xhr);
      this.xhr.send(null);
    },
